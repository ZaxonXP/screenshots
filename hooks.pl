...
=cut
	...
	$search_prefixes{'^(du|ddg|duck)(| .*)$'}                  = 'duckduckgo'; # DuckDuckGo
    $search_prefixes{'^(wpl|wikipl)(| .*)$'}                   = 'wikipl';     # Wikipedia PL
	...
	sub search
	{
		...
		(
			"wikipl"     => {
				home     => 'https://pl.wikipedia.org/',
				search   => 'https://pl.wikipedia.org/?q='},
			"duckduckgo" => {
...
Remove header and footer parts (leave article and comments)
...
=cut
    ...
    {
        my $free = "~<br>" x 42;
        $html =~ s/<head>.+(<title>.+<\/title>).+<\/head>/<head>$1<\/head>/sm;
        $html =~ s/<div class="news-article--categories">.+?Less tags<\/button>.+?<\/div>//sm;
        $html =~ s/<script>.+?<\/script>//sm;
        $html =~ s/<a href="#main-content".+skip-link">.+?<div class="author-.+?"><\/div>//sm;
        $html =~ s/<div class="rrssb-prefix">Share this:<\/div>//sm;
        $html =~ s/<ul class="rrssb-buttons">.+?<\/ul>//sm;
        $html =~ s/View this post on Instagram.*//sm;
        $html =~ s/<h2 class="block-title">Follow us:<\/h2>.+/<div style="justify-content:left;">$free<\/div>/sm;
        $html =~ s/<p><\/p><blockquote class="instagram-media".*/<div style="justify-content:left;">$free<\/div>/sm;
    }
...
=cut
    ...
    if ($url =~ 'dutchnews\.nl')
    {
        my $free = "~<br>" x 42;
        $html =~ s/<head>.+(<title>.+<\/title>).+<\/head>/<head>$1<\/head>/sm;
        $html =~ s/<\/head>.+<div class="date lead text-nowrap">(.+?)<\/div>.+<header>/<header>$1/sm;
        $html =~ s/<div class="post-meta mb-3"> <a class="badge.+?Photo: .+?<\/p><\/div>//sm;
        $html =~ s/<h4>Thank you for donating.+/<div style="justify-content:left;">$free<\/div>/sm;
    }
...
=cut
    if ($url =~ 'wolnemedia\.net')
    {
        my $free = "~<br>";
        $html =~ s/.*(<title>.+<\/title>).*id="post-\d+">/$1/sm;
        $html =~ s/<div id="print-box">.*(<div id="comments">)/$1/sm;
        $html =~ s/<div id="ai_post_reaction_main".*(<div id="comments">)/$1/sm;
        $html =~ s/<div class="wpra-call-to-action".*(<div id="comments">)/$1/sm;
        $html =~ s/<span>Polecaj.+? to<\/span>//sm;
        $html =~ s/<span>Polecisz to.+?<\/span>//sm;
        $html =~ s/<div id="respond">.*/$free/sm;
    }
...
=item Swiadomosc Zwiazkow

Remove header and footer parts (leave article and comments)

=cut

    if ($url =~ 'swiadomosc-zwiazkow\.pl')
    {
        my $free = "~<br>" x 30;

        $html =~ s/.*(<header class="entry-header">)/$1/sm;
        #$html =~ s/<div id="sidebar-right">.*/$free/sm;
        #$html =~ s/<font size="2"><strong>OD ADMINISTRATORA PORTALU:.*//g;
        #$html =~ s/<br>Pogl.+dy wyra.+ane.*//g;
        #$html =~ s/<div id="downloadpdf".*//g;
        #$html =~ s/Przeczytano \d+&nbsp;razy\!//; 
        #$html =~ s/<img src=".*loading\.gif.*<\/div>.*//g;
        #$html =~ s/<div id="post-ratings-\d+".*//g;
    }
